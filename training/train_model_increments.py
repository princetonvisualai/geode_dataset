...
region_dset = pickle.load(open('.../geode_prep_africa.pkl', 'rb')) # change per region
imagenet_dset = pickle.load(open('.../imagenet_prep.pkl', 'rb'))
imagenet_indexsample = pickle.load(open('.../imagenet_33383indices.pkl', 'rb'))
imagenet_dset_excluded = pickle.load(open('.../imagenet33383_excludedIndices.pkl', 'rb'))
...
ratio = 0.05 # change per increment

print("Percentage of GeoDE Images: " + str(ratio))

full_region_random_sample = np.random.choice(len(region_dset['train'][0]), region_num_images, replace=False)

region_num_images = np.floor(region_num_images * ratio).astype(int)
region_random_sample = np.random.choice(full_region_random_sample, region_num_images, replace=False) # gives indices corresponding to region_dset

excluded_pics = np.array([i for i in full_region_random_sample if i not in region_random_sample]) # geode excluded from training from orginal 4970

checkpoint1 = len(full_region_random_sample) - len(region_random_sample)

print("Number of Images Taken out of GeoDE Checkpoint 1: " + str(checkpoint1))
print("Number of Images Taken out of GeoDE Checkpoint 2: " + str(len(excluded_pics)))

imagenet_addOn = set() # imagenet images added on to supplement geode being taken out
imagenet_excluded = set() # images excluded from 33383 set of imagenet
for obj in range(38):
    for image in imagenet_dset_excluded[str(obj)][0]:
        imagenet_excluded.add(image)
    excluded = set() 
    for pic_index in excluded_pics: # for each index in pics excluded from geode 4970
...
print("Number of Images Taken out of GeoDE Checkpoint 3: " + str(len(imagenet_addOn)))
...
save_dir = 'record/train_on_{}'.format("pretrained_5per_imagenet33383_africa38_50epochs") #change per region and per increment
test_dir = 'record/train_on_{}'.format("pretrained_5per_imagenet33383_africa38_50epochs") #change per region and per increment
...
if args.test:
    ...
    europe_dset = pickle.load(open('.../geode_prep_europe.pkl', 'rb'))
