...
geode_dset = pickle.load(open('data/geode29_prep.pkl', 'rb'))
...
geode_train_names = geode_dset['train'][0]
geode_train_obj = geode_dset['train'][1]
print("GeoDE Training Size: " + str(len(geode_train_names)))
...
train_names = [*imagenet_train_names, *geode_train_names]
train_obj = [*imagenet_train_obj, *geode_train_obj]
print("Total Training Size: " + str(len(train_names)))

val_names = [*imagenet_dset['val'][0], *geode_dset['val'][0]]
val_obj = [*imagenet_dset['val'][1], *geode_dset['val'][1]]
...
save_dir = 'record/train_on_{}'.format("pretrained_geode_geoyfcc_50epochs")
test_dir = 'record/test_on_{}'.format("pretrained_geode_geoyfcc_50epochs")
...
if args.test:
    ...
    geode_dset_test = load_data.ImageDatasetMultiLabel(*geode_dset['test'], transform = transform_test)
    geode_loader_test = load_data.DataLoader(geode_dset_test, **params_valtest)

    geode_all_scores = []
    geode_all_targets = []
    with torch.no_grad():
        for (x,y) in geode_loader_test:
            x = x.to(device=device, dtype=dtype)
            y = y.to(device=device, dtype=ydtype)
                
            scores = model(x)  
            geode_all_scores.append(scores.detach().cpu().numpy())
            geode_all_targets.append(y.detach().cpu().numpy())

    geode_all_targets = np.concatenate(geode_all_targets)
    geode_all_scores = np.concatenate(geode_all_scores)
    acc, _ = compute_acc(geode_all_targets,geode_all_scores)
    print(acc)
